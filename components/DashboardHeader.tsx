...
const DashboardHeader: React.FC<DashboardHeaderProps> = ({ data }) => {
    const qrValue = encodeURIComponent(`https://chargeup.in/passport/${data.bpan}`);
    const qrSrc = `https://api.qrserver.com/v1/create-qr-code/?data=${qrValue}&size=108x108&color=0f172a&bgcolor=ffffff&margin=4`;
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between mb-4">
                        <div>
                            <p className="text-slate-500 font-medium text-sm">
                                BPAN:{" "}
                                <span className="font-mono text-slate-300 font-bold bg-white/5 border border-white/10 px-2 py-0.5 rounded">
                                    {data.bpan}
                                </span>
                            </p>
                        </div>
